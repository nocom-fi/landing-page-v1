  ...
  return (
    <form onSubmit={handleSubmit} className="relative w-full max-w-md mx-auto">
      {isSubmitted ? (
        ...
        <div className="relative flex items-center bg-[#0F0F11]/80 backdrop-blur-sm border border-[#2A2A2E] rounded-full p-1.5 pl-5 transition-all duration-300 hover:border-white/40 group">
          <input
            type="email"
            value={email}
            onChange={(e) => setEmail(e.target.value)}
            placeholder="email@nocom.finance"
            className="flex-1 bg-transparent border-none outline-none text-white placeholder-[#8B8B9B] text-sm font-sans tracking-wide"
            required
          />
          <button
            type="submit"
